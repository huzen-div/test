...
<div id="all" class="tabcontent" style="padding-top: 0; display:block;">
    ...
    <form method="post" action="<?= base_url('supplies/edit_requisition') . '/' . $data[0]['id'] ?>">
        ...
        <div class="row" style="margin-bottom: 1%;">
            ...
            <div class="col-md-6">
                ...
                <select class="form-control" name="status" id="status">
                    ...
                    <option value="1" <?php if ($data[0]['status'] == 1) echo 'selected'; ?>>จ่ายแล้ว</option>
                    <option value="2" <?php if ($data[0]['status'] == 2) echo 'selected'; ?>>ระหว่างดำเนินการ</option>
                    <option value="3" <?php if ($data[0]['status'] == 3) echo 'selected'; ?>>ยกเลิก</option>
                    <!-- <option value="4" <?php if ($data[0]['status'] == 4) echo 'selected'; ?>>เรียบร้อย</option> -->
                </select>
            </div>
        </div>
        ...
        <div class="row" style="margin-bottom: 1%;">
            ...
            <div class="col-md-6">
                <label for="tax_rate">รวมภาษี 7%</label>
                <select class="form-control" name="tax_rate" id="tax_rate" required>
                    <option selected value="">เลือกรวมภาษี 7%</option>
                    <?php 
                    foreach ($taxrate as $row) { ?>
                        <option value="<?= $row['id'] ?>" <?php $data[0]['tax_rate'] == $row['id'] ? print 'selected' : ''; ?>><?= $row['name'] ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
