...
<div id="all" class="tabcontent" style="padding-top: 0; display:block;">
    <form action="<?= base_url('finance/actions_billing') ?>" method="POST">
        ...
        <input type="button" class="btn btn-dark" onclick="myFunction1()" value="Import Excell" style="margin-left: 2%;" />
    </div> -->
        <div class="row" style="margin-bottom: 1%;">
        ...
        <input type="button" class="btn btn-primary offset-md-2" onclick="myFunction()" value="เรียกเก็บค่าสมาชิก" style="margin-left: 2%;" />
    -->
        </div>
        <hr>
                ...
                <?php if ($data) : ?>
                    <?php foreach ($data as $x) : ?>
                        <tr>
                            ...
                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal"><?php echo 'MOPH-' . sprintf('%07d', $x['customer_id']); ?></td>

                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal"><?php echo $x['name']; ?></td>
                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal">

                                <?php echo Date('d/m/Y', strtotime($x['date'])); ?>
                                <!-- <?php echo thaidate(date('d/m/Y', strtotime($x['date']))); ?> -->

                            </td>
                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum">
                                <center><?php echo $x['day']; ?></center>
                            </td>
                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum"><?php echo number_format($x['amount'], 2); ?></td>
                            <?php
                            $vat = $x['amount'] * 0.07;
                            ?>

                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum"><?php echo number_format($vat, 2); ?></scenter>
                            </td>
                            <td href="<?= base_url('finance/view_billing_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum"><?php echo number_format($x['amount'] + $vat, 2); ?></td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <a class="dropdown-item" href="<?= base_url('finance/view_billing') . '/' . $x['id'] ?>">รายละเอียด<?= $title ?></a>
                                    <a class="dropdown-item" href="<?= base_url('finance/edit_billing') . '/' . $x['id'] ?>">แก้ไข<?= $title ?></a>
                                    <a class="dropdown-item" href="<?= base_url('finance/delete_billing') . '/' . $x['id'] ?>"onclick="return confirm('ยืนยัน ?')">ลบ<?= $title ?></a>
                                </div> -->
                                    <div class="dropdown-menu" aria-labelledby="actions">
...
<script>
    function myFunction() {
        // location.replace("/finance/billing")
        // location("/finance/billing")
        window.location = "/finance/billing";

    }

    function myFunction1() {
        window.location = "/finance/import_excell";
