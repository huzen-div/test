...
// include ("vendor/phpbarcodegeneratorsrc/BarcodeGenerator.php");
// include ("vendor/phpbarcodegeneratorsrc/BarcodeGeneratorHTML.php");
require_once(APPPATH.'vendor/phpbarcodegeneratorsrc/BarcodeGenerator.php');
require_once(APPPATH.'vendor/phpbarcodegeneratorsrc/BarcodeGeneratorHTML.php');
...
class Supplies extends Controller
{
	...
	public function list_product($status = null)
	{
		...
		$data['data'] = $this->supplie_model->getProduct($status);
		$data['title'] = 'จัดการคลังสินค้า';
	...
	public function product()
	{
		if ($this->request->getPost('save') != null) {
			...
			$data['pages'] = [['link' => base_url('supplies/index'), 'title' => 'ภาพรวมงานพัสดุ'], ['link' => base_url('supplies/list_product'), 'title' => 'จัดการคลังสินค้า'], ['link' => '#', 'title' => $data['title']]];
			echo view('header', $data);
	...
	public function view_product($id = null)
	{
		...
		$data['category_sub'] = $this->supplie_model->getSubCategory($data['data'][0]['category_minor_id']);
	...
	public function actions_product()
	{
		if (!empty($_POST['val'])) {
			...
			if ($this->request->getPost('excell_x')) {
				...
				$header = array("ลำดับ", "รหัส", "ชื่อสินค้า", "รหัสสินค้า", "ประเภท", "สถานะ", "หมวดหมู่หลัก", "หมวดหมู่ย่อย", "หน่วยนับ", "ผู้รับผิดชอบ", "ราคา", "รวมภาษี", "หมายเหตุ", "วันที่");
				fputcsv($file, $header);
				$no = 1;
				$ar = [
					'0' => '',
	...
	{
		$data['data'] = $this->supplie_model->getCheckStock();
		$data['title'] = 'นับสต๊อก';
		$data['pages'] = [['link' => base_url('supplies/index'), 'title' => 'ภาพรวมงานพัสดุ'], ['link' => '#', 'title' => $data['title']]];
		echo view('header', $data);
	...
	public function view_check_stock_id($id = null)
	{
		if ($id != null) {
			$data['data'] = $this->supplie_model->getCheckStockByID($id);
			$data['title'] = 'รายละเอียดนับสต๊อก';
	...
	{
		$data['data'] = $this->supplie_model->getReceiveSupplies();
	...
	public function actions_borrow()
	{
		if (!empty($_POST['val'])) {
			...
			if ($this->request->getPost('excell_x')) {
				...
				$file = fopen('php://output', 'w');
				fputs($file, (chr(0xEF) . chr(0xBB) . chr(0xBF)));
				$header = array("ลำดับ", "รหัส", "วันที่", "เลขที่เอกสาร", "ผู้เบิก", "แผนก", "สถานะ", "Return note", "Staff note");
				fputcsv($file, $header);
				$no = 1;
				foreach ($_POST['val'] as $id) {
					// get data 
	...
	public function depreciation()
	{
		if ($this->request->getPost('asset') != null) {
			...
			$data['asset'] = $this->supplie_model->getAsset();
	...
	public function list_registration_bring_forward()
	{
		if (!empty($_POST['val'])) {
			if ($this->request->getPost('excell_x')) {
				// var_dump($_POST['val']);
				// file name 
				$filename = 'registration_bring_forward_' . date('Y_m_d_H_i_s') . '.csv';
				header("Content-Description: File Transfer");
				header("Content-Disposition: attachment; filename=$filename");
				header("Content-Type: application/csv; ");
				$file = fopen('php://output', 'w');
	...
	public function list_registration_depreciation()
	{
		if (!empty($_POST['val'])) {
			if ($this->request->getPost('excell_x')) {
				// var_dump($_POST['val']);
				// file name 
				$filename = 'registration_depreciation_' . date('Y_m_d_H_i_s') . '.csv';
				header("Content-Description: File Transfer");
				header("Content-Disposition: attachment; filename=$filename");
				header("Content-Type: application/csv; ");
				$file = fopen('php://output', 'w');
	...
	public function list_registration_transfer_repair()
	{
		if (!empty($_POST['val'])) {
			if ($this->request->getPost('excell_x')) {
				// var_dump($_POST['val']);
				// file name 
				$filename = 'registration_transfer_repair_' . date('Y_m_d_H_i_s') . '.csv';
				header("Content-Description: File Transfer");
				header("Content-Disposition: attachment; filename=$filename");
				header("Content-Type: application/csv; ");
				$file = fopen('php://output', 'w');
