...
<div id="all" class="tabcontent" style="padding-top: 0; display:block;">
    <form action="<?= base_url('finance/actions_pettycash') ?>" method="POST">
        ...
        <div class="row search_tab" style="margin-bottom: 1%;">
            ...
            <input type="button" class="btn btn-warning offset-md-3" onclick="myFunction()" value="บันทึกเบิกเงินสดย่อย" style="margin-left: 2%;" />
            ...
            <tbody>
                <?php if ($data) : ?>
                    <?php foreach ($data as $x) : ?>
                        <tr>
                            <td><?php echo $x['id']; ?></td>
                            <td href="<?= base_url('finance/view_pettycash_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum">
                                <center><?php echo $x['type_id']; ?></center>
                            </td>
                            <td href="<?= base_url('finance/view_pettycash_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum">
                                <center><?php echo $x['status_id']; ?></center>
                            </td>
                            <td href="<?= base_url('finance/view_pettycash_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum">
                                <center><?php echo $x['no_id']; ?></center>
                            </td>
                            <td href="<?= base_url('finance/view_pettycash_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal">
                                <!-- <?php echo thaidate(date('d/m/Y', strtotime($x['date']))); ?> -->
                                <?php echo date('d/m/Y', strtotime($x['date'])); ?>
                            </td>
                            <!-- <td><?php echo $x['date']; ?></td> -->
                            <td href="<?= base_url('finance/view_pettycash_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum">
                                <center><?php echo $x['withdrawal_slip']; ?></center>
                            </td>
                            <td href="<?= base_url('finance/view_pettycash_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum"><?php echo number_format($x['amount'], 2); ?></td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="actions">
                                        <a class="dropdown-item" href="<?= base_url('finance/view_pettycash') . '/' . $x['id'] ?>">รายละเอียด</a>
                                        <a class="dropdown-item" href="<?= base_url('finance/edit_pettycash') . '/' . $x['id'] ?>">แก้ไข</a>
                                        <a class="dropdown-item" href="<?= base_url('finance/delete_pettycash') . '/' . $x['id'] ?>" onclick="return confirm('คุณต้องการลบข้อมูลหรือไม่ ?')">ลบ</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
...
<script>
    function myFunction() {
        window.location = "/finance/pettycash";
        // location.replace("/finance/pettycash")
    }
</script>