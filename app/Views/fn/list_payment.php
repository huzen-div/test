...
<form action="<?= base_url('finance/actions_payment') ?>" method="POST">
...
<label id="date-label-from" class="col-md-1">ค้นหา </label>
    ...
    <div class="row" style="margin-bottom: 1%;">
        ...
        <input type="button" class="btn btn-primary offset-md-2" onclick="myFunction()" value="เพิ่ม<?= $title ?>" style="margin-left: 2%;" />
    </div>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="actions">
                                    <a class="dropdown-item" href="<?= base_url('finance/view_payment') . '/' . $x['id'] ?>">รายละเอียด<?= $title ?></a>
                                    <a class="dropdown-item" href="<?= base_url('finance/edit_payment') . '/' . $x['id'] ?>">แก้ไข<?= $title ?></a>
                                    <a class="dropdown-item" href="<?= base_url('finance/delete_payment') . '/' . $x['id'] ?>"onclick="return confirm('ยืนยัน ?')">ลบ<?= $title ?></a>
                                </div>
                            </div>
                        </td>
                    </tr>
...
<script>
    function myFunction() {
        location.replace("/finance/payment")
    }
</script>