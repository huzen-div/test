...
class Supplies extends Controller
{
	...
	public function product()
	{
		if ($this->request->getPost('save') != null) {
			...
			$add = [
				...
				'category_minor_id'	=> $this->request->getPost('category_minor_id') ? $this->request->getPost('category_minor_id') : null,
	...
	public function receive_supplies()
	{
		if ($this->request->getPost('save') != null) {
			...
			$this->supplie_model->addLog(1);
			return redirect()->to('/supplies/receive_supplies');
		} else {
	...
	public function actions_borrow()
	{
		if (!empty($_POST['val'])) {
			...
			if ($this->request->getPost('excell_x')) {
				...
				$file = fopen('php://output', 'w');
				fputs($file, (chr(0xEF) . chr(0xBB) . chr(0xBF)));
				$header = array("ลำดับ", "รหัส", "วันที่", "เลขที่เอกสาร", "ผู้เบิก", "แผนก", "สภานะ", "Return note", "Staff note");
				fputcsv($file, $header);
				$no = 1;
				foreach ($_POST['val'] as $id) {
					// get data 
	...
	public function depreciation()
	{
		if ($this->request->getPost('save') != null) {
			...
			$add = [
				...
				'year'		=> intval($this->request->getPost('year')) != 0 ? intval($this->request->getPost('year')) : 1,
				'price'   	=> $this->request->getPost('price') ? (float)str_replace(",", "", $this->request->getPost('price')) : 0,
