...
<div id="all" class="tabcontent" style="padding-top: 0; display:block;">
    <form action="<?= base_url('account/list_payjournal') ?>" method="POST">
                ...
                <?php if ($data) : ?>
                    <?php foreach ($data as $x) : ?>
                        <tr>
                            ...
                            <td><?php echo $x['id']; ?></td>
                            <td href="<?= base_url('account/view_payjournal_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal"><?php echo $x['no_id']; ?></td>
                            <td href="<?= base_url('account/view_payjournal_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal"><?php echo date('d/m/Y', strtotime($x['date'])); ?></td>
                            <!-- <td><?php echo $x['date']; ?></td> -->
                            <td href="<?= base_url('account/view_payjournal_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal"><?php echo $x['refer']; ?></td>
                            <td href="<?= base_url('account/view_payjournal_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal"><?php echo $x['detail']; ?></td>
                            <td href="<?= base_url('account/view_payjournal_id') . '/' . $x['id'] ?>" data-toggle="modal" data-remote="false" data-target="#myModal" class="textnum"><?php echo number_format($x['amount'], 2); ?></td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="actions">
                                        <a class="dropdown-item" href="<?= base_url('account/view_payjournal') . '/' . $x['id'] ?>">รายละเอียด<?= $title ?></a>
                                        <a class="dropdown-item" href="<?= base_url('account/edit_payjournal') . '/' . $x['id'] ?>">แก้ไข<?= $title ?></a>
                                        <a class="dropdown-item" href="<?= base_url('account/delete_payjournal') . '/' . $x['id'] ?>" onclick="return confirm('ยืนยัน ?')">ลบ<?= $title ?></a>
                                    </div>
                                </div>
                            </td>
                        </tr>
...
<script>
    function myFunction() {
        window.location = "/account/payjournal";
        // location.replace("/account/payjournal")
    }
</script>